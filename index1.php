<?php
// =========================================================
// index.php (V1.0 - Main Application Dashboard)
// =========================================================
 ...
 * - Refreshes permanent tasks if they were not completed yesterday.
 * - Penalizes points if daily task count was not met (if feature was implemented).
 * - Resets the daily task completed counter.
 */
function checkDailyReset(&$user, $dbManager) {
    ...
    if ($user['last_task_refresh'] < $today_midnight_ts) {
        ...
        foreach ($tasks as $task) {
            ...
            if ($task['permanent'] === true && $task['completed'] === false) {
                // If permanent task was NOT completed, it remains incomplete and NO penalty.
                // The task is simply carried over as incomplete.
                $updatedTasks[] = $task;
            } elseif ($task['permanent'] === true && $task['completed'] === true) {
                // If permanent task was completed, reset its status for the new day
                $task['completed'] = false;
                $updatedTasks[] = $task;
            } elseif ($task['permanent'] === false && $task['completed'] === false) {
                // Non-permanent, incomplete tasks are deleted at the end of the day.
                // We do NOT add them to the updatedTasks list.
            } else {
                // Non-permanent, completed tasks are also removed.
                // The points are already added, so the task is complete and temporary.
            }
        }
        ...
        // Rank update and save is handled by updateUserData which is called later
    }
}
...
function handleSpCollect(&$user, $dbManager) {
    ...
    if ($user['last_sp_collect'] >= $today_midnight_ts) {
        ...
        echo json_encode(['success' => false, 'message' => 'Error: Daily SP already collected!']);
        return;
    }

    $user['sp_points'] += DAILY_CHECKIN_REWARD;
    $user['last_sp_collect'] = time();
    $message = "SP COLLECTED! +".DAILY_CHECKIN_REWARD." SP. Total: {$user['sp_points']}";
...
function handleTaskActions(&$user, $dbManager) {
    ...
    $isPermanent = (($_POST['permanent'] ?? 'false') === 'true'); // PHP handles string conversion from JS boolean
        ...
        foreach ($tasks as $key => &$task) {
            if ($task['id'] === $taskId) {
                ...
                if ($action === 'set_permanent') {
                    // Update task status and save the data
...
function generateHtml($user, $dbManager) {
    ...
    $spButtonText = $canCollectSp ? 'COLLECT Daily SP' : 'SP Already Collected Today';
...
<div class="header-bar">
    ...
    <div class="header-menu">
        ...
        <div id="dropdown-menu" class="menu-dropdown">
            ...
            <span class="dropdown-sp-collected">SP: <span id="sp-display-menu"><?php echo $user['sp_points']; ?></span></span>
...
<div class="container">
    <div class="profile-container">
        <h2>Commander: <?php echo htmlspecialchars($user['username']); ?></h2>
        <div class="stats-line">
            Total Task Points (TP): <strong id="tp-stats"><?php echo $user['task_points']; ?></strong>
        </div>
        <div class="stats-line">
            Total Self-Improvement Points (SP): <strong id="sp-stats"><?php echo $user['sp_points']; ?></strong>
        </div>
        <div class="stats-line">
            Daily Tasks Completed: <strong id="daily-count-stats"><?php echo $user['daily_completed_count']; ?></strong>
        </div>
...
<script>
    ...
    function renderTaskHtml(task) {
        ...
        return `
            <div class="task-slot ${completedClass}" id="task-${task.id}" data-id="${task.id}" data-permanent="${task.permanent}" data-completed="${task.completed}">
                ...
                <button class="remove-btn" onclick="deleteTask('${task.id}')">X Delete</button>
            </div>
        `;
    }
    ...
    async function deleteTask(id) {
        if (!confirm("Confirm mission abort (delete)?")) return;
    ...
    async function collectSp() {
        ...
        if (result.success) {
            ...
            button.textContent = 'SP Already Collected Today';
        } else {
            alert(result.message);
    ...
    async function saveObjective() {
        ...
        if (result.success) {
            ...
            // Input field already reflects the value, no need to update
        } else {
            alert(result.message);
        }
    }
